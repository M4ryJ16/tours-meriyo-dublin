...
import { ApiError, getBooking, capturePaypalOrder, getCurrentCustomer } from '@/lib/api.js';
...
export const POST: APIRoute = async ({ params, request, cookies }) => {
  ...
  if (!body.orderId) {
    return json({ message: 'Falta orderId.' }, 400);
  }
  ...
  try {
    ...
    const updated = await capturePaypalOrder(params.id as string, body.orderId);
