...
import { ApiError, getBooking, createPaypalOrder, getCurrentCustomer } from '@/lib/api.js';
...
export const POST: APIRoute = async ({ params, cookies }) => {
  ...
  try {
    ...
    const order = await createPaypalOrder(params.id as string);
    ...
    return json({ message: 'No se ha podido iniciar el pago con PayPal. Inténtalo de nuevo más tarde.' }, 500);
  }
};